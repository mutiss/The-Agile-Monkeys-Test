...
class PlayerHolder(
    ...
    // Prepare playback.
    fun start(previewUrl:String) {
        var source: ExtractorMediaSource? = null
        var uri: Uri? = null
        actualUrl = previewUrl
        audioFocusPlayer.prepare(buildMediaSource(Uri.parse(actualUrl)))
        ...
        if (uri != null) {
            ...
            // Load media.
            audioFocusPlayer.prepare(source)
        }
